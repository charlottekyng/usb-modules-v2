...
brass : $(foreach pair,$(SAMPLE_PAIRS),brass/$(pair)/$(tumor.$(pair))_vs_$(normal.$(pair)).intermediates.tar.gz)
#brass : $(foreach pair,$(SAMPLE_PAIRS),brass/$(pair).brass_wrap.log)

bam/%.bam.bas : bam/%.bam
	$(call RUN,1,6G,$(RESOURCE_REQ_MEDIUM),$(SINGULARITY_MODULE),"\
	$(BRASS) bam_stats -i $< -o $@ -r $(REF_FASTA).fai")

brass_files/%.bw : bam/%.bam
	$(MKDIR) $(@D)
	$(call RUN,1,12G,$(RESOURCE_REQ_MEDIUM),$(SINGULARITY_MODULE),"\
	$(CPGBIGWIG) bam2bw -i $< -o $@")

brass_files/depth/%.bed : brass_files/%.bw
	$(MKDIR) $(@D)
	$(call RUN,1,6G,$(RESOURCE_REQ_MEDIUM),$(SINGULARITY_MODULE),"\
	$(CPGBIGWIG) detectExtremeDepth -b $< -o brass_files/depth/ ")
...
ifneq ($(words $(foreach pair,$(SAMPLE_PAIRS),brass_files/sorted/$(tumor.$(pair)).bed)),1)
...
brass/$1_$2/$1_vs_$2.intermediates.tar.gz : bam/$1.bam bam/$2.bam bam/$1.bam.bas bam/$2.bam.bas brass_files/merged/depth_mask.bed brass_files/sampstat/$1.txt
	$$(call RUN,2,64G,$$(RESOURCE_REQ_MEDIUM),$$(SINGULARITY_MODULE),"\
	$$(BRASS) brass.pl $$(BRASS_OPTS) \
	-ss $$(<<<<<<) -t $$< -n $$(<<) -tn $1 -nn $2 -c 2 -o brass/$1_$2 && touch $$@")
