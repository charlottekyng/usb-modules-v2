...
conSV : $(foreach pair,$(SAMPLE_PAIRS),conSV/conSV_$(MIN_CALLERS)_outof_$(words $(SV_CALLERS))/$(pair).$(MIN_CALLERS)_outof_$(words $(SV_CALLERS))_slope_$(SLOPE).tsv)
conSV_disruption : $(foreach pair,$(SAMPLE_PAIRS),conSV/conSV_$(MIN_CALLERS)_outof_$(words $(SV_CALLERS))/$(pair).$(MIN_CALLERS)_outof_$(words $(SV_CALLERS))_slope_$(SLOPE)_gene_disruption.tsv)
conSV_fusion : $(foreach pair,$(SAMPLE_PAIRS),conSV/conSV_$(MIN_CALLERS)_outof_$(words $(SV_CALLERS))/$(pair).$(MIN_CALLERS)_outof_$(words $(SV_CALLERS))_slope_$(SLOPE)_fusion.tsv)

# create folder and organise data
# BRASS no filter implemented so taken as it is
define preprocess_brass
conSV/brass/$1_$2.vcf : brass/$1_$2/$1_vs_$2.annot.vcf.gz
	$$(MKDIR) $$(@D)
...
conSV/delly/$1_$2.vcf : delly/$1_$2.somatic.SVpass.vcf
	$$(MKDIR) $$(@D)
	$$(call RUN,1,$$(RESOURCE_REQ_LOW_MEM),$$(RESOURCE_REQ_VSHORT),,"\
	ln -f $$< $$@")
endef
...
conSV/gridss/$1_$2.vcf : gridss/$1_$2.somatic.SVpass.vcf
	$$(MKDIR) $$(@D)
	$$(call RUN,1,$$(RESOURCE_REQ_LOW_MEM),$$(RESOURCE_REQ_VSHORT),,"\
	ln -f $$< $$@")
endef
...
conSV/manta/$1_$2.vcf : manta/$1_$2/results/variants/somaticSV.SVpass.vcf
	$$(MKDIR) $$(@D)
	$$(call RUN,1,$$(RESOURCE_REQ_LOW_MEM),$$(RESOURCE_REQ_VSHORT),,"\
	ln -f $$< $$@")
endef
...
conSV/svaba/$1_$2.vcf : svaba/$1_$2.svaba.somatic.sv.SVpass.vcf
	$$(MKDIR) $$(@D)
	$$(call RUN,1,$$(RESOURCE_REQ_LOW_MEM),$$(RESOURCE_REQ_VSHORT),,"\
	ln -f $$< $$@")
endef

# Preprocess step for SV callers to populate the folder and rename files appropriately
...
$(foreach pair,$(SAMPLE_PAIRS), \
  $(foreach caller,$(SV_CALLERS), \
    $(eval $(call variant_preprocess_varExtr,$(tumor.$(pair)),$(normal.$(pair)),$(caller))) \
  ) \
)
