...
#		lrr <- lrr[round(0.25*length(lrr)):round(0.75*length(lrr))]
		lrr <- lrr[round(0.4*length(lrr)):round(0.6*length(lrr))]

		df$GL_LRR <- 0
		df$GL_LRR[df$log2 < median(lrr)-(0.8*sd(lrr))] <- -1
		df$GL_LRR[df$log2 < median(lrr)-(2.5*sd(lrr))] <- -2
		df$GL_LRR[df$log2 > median(lrr)+(1*sd(lrr))] <- 1
		df$GL_LRR[df$log2 > median(lrr)+(2*sd(lrr))] <- 2
...
mm <- lapply(mm, function(x){
	x[match(genes$gene, x$gene),-c(1:5)]
})
