...
plot_heatmap <- function(facets_tab, plot_file, sample_names=NULL, col=c("red", "darksalmon", NA, "lightblue", "blue"), zlim=c(-2,2)) {
    ...
    pdf(plot_file, width=7, height=3*length(sample_names))
...
#    axis(1,at=chrmid/(max(chrsep)-1), label=rle(mm$chrom)$values, cex.axis=0.8, tick=F, family=fontfamily)
    axis(2,at=seq(0,1,1/max((ncol(mm2)-1),1)), label=unlist(lapply(colnames(mm2), function(x){strsplit(x,split="_")[[1]][1]})), las=2, cex.axis=1, tick=F)
    }, mm)
    legend("bottom", inset=c(0,-1.2), legend=c("Homozygous deletion", "Loss", "Gain", "Amplification"),
