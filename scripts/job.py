...
class SLURMJob(ClusterJob):
    ...
    def run_job(self):
        ...
        self.jobid_file = tempfile.NamedTemporaryFile(mode='w', suffix='.id', delete=False)
        os.chmod(self.jobid_file.name, 0o666)

        ''' The -W switch is wait '''
        cmd = "sbatch -W {args} {script}".format(args=self.qsub_args, script=self.job_script_file.name)
...
class SGEJob(ClusterJob):
    ...
    def run_job(self):
        ...
        cmd = "qsub -sync y {args} {script}".format(args=self.qsub_args, script=self.job_script_file.name)
        self.process = subprocess.Popen(cmd, stdout=subprocess.PIPE, shell=True)
